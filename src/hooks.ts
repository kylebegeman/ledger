...
/** Project-relative path of the script Codex hooks run when installed with `--launcher`. */
export const hookLauncherPath = ".ledger/bin/ledger.mjs";

/** The command prefix of hooks that run the launcher. */
export const hookLauncherCommand = `node ${hookLauncherPath}`;

const codexTrustStep =
...
const codexKeptTrustStep =
  ".codex/hooks.json did not change, so hooks already approved in Codex stay approved.";
...
const hostHookFiles: Record<LedgerHookHost, Omit<HostHookFile, "host">> = {
  ...
  codex: {
    ...
    nextSteps: [
      codexTrustStep,
      "Approve them again whenever the hook file changes, such as a new pinned version; Codex skips a changed hook until it is trusted again. Install with --launcher to keep the hook file the same across versions.",
    ],
  },
  cursor: {
...
const codexLauncherSteps = [
  codexTrustStep,
  `The hooks run ${hookLauncherPath}, so a new pinned version rewrites only that script and Codex keeps its approval. Start Codex at the project root, where the hooks find the script.`,
];

/**
 * The host's hook file description with next steps rendered for the command
 * that runs Ledger, or for the launcher when Codex hooks run it.
 */
export function hostHookFile(host: LedgerHookHost, command = defaultAgentsCommand, launcher = false): HostHookFile {
  const file = hostHookFiles[host];
  const viaLauncher = launcher && host === "codex";
  const runs = viaLauncher ? hookLauncherCommand : command;
  const hint = runs === defaultAgentsCommand
  ...
  return {
    ...
    events: file.events,
    nextSteps: [
      `Confirm the hook command resolves to this Ledger: \`${runs} version\` should print the package version.${hint}`,
      ...(viaLauncher ? codexLauncherSteps : file.nextSteps),
    ],
  };
}

/**
 * The launcher Codex hooks run when installed with `--launcher`. It runs
 * `command` through the shell from the project root, passes the hook's
 * arguments, stdin, and stdout through, and exits with the command's status.
 * Codex approves hooks by their definition, so a new version rewrites this
 * script while the hook file, and the approval, stay the same. When the
 * shell cannot start, the script prints the empty object and exits 0, as
 * `ledger hook` does, so the host is never blocked.
 */
export function renderHookLauncher(command: string): string {
  return [
    "// Generated by ledger hooks install --host codex --launcher. Rerun that command instead of editing.",
    "// Codex hooks run this script, so a new Ledger version rewrites only the command below and",
    "// .codex/hooks.json keeps the hooks Codex approved.",
...
const hookLauncherBody = String.raw`const root = fileURLToPath(new URL("../..", import.meta.url));
const line = [command, ...process.argv.slice(2).map(quote)].join(" ");
const child = spawn(line, { cwd: root, shell: true, stdio: "inherit" });
for (const signal of ["SIGINT", "SIGTERM"]) process.on(signal, () => child.kill(signal));
child.on("error", (error) => {
  console.error("ledger launcher: cannot run " + command + ": " + error.message);
  console.log("{}");
});
...
/** Whether a launcher script is the one Ledger writes for `command`, ignoring line endings. */
export function isCurrentHookLauncher(script: string, command: string): boolean {
  return script.replace(/\r\n/g, "\n") === renderHookLauncher(command);
}

/** The command a launcher script runs, or undefined when Ledger did not write the script. */
export function hookLauncherTarget(script: string): string | undefined {
  if (!script.startsWith("// Generated by ledger hooks install")) return undefined;
  const literal = /^const command = (".*");\r?$/m.exec(script)?.[1];
...
/** The command prefixes of the Ledger hook entries anywhere in a parsed hook file. */
export function ledgerHookPrefixes(value: unknown): readonly string[] {
...
export interface InstallHooksOptions {
  ...
  /**
   * Run Codex hooks through the launcher (true) or the command itself
   * (false). Defaults to the form the hook file already uses. Codex only.
   */
  readonly launcher?: boolean;
  readonly dryRun: boolean;
}

/** The launcher after an install: already current, written, or removed when the hooks stop using it. */
export interface HookLauncherResult {
  readonly path: string;
  readonly state: "current" | "written" | "removed";
}
...
export interface InstallHooksResult {
  ...
  /** The launcher the Codex hooks run, present when they run it or stopped running it in this install. */
  readonly launcher?: HookLauncherResult;
  readonly nextSteps: readonly string[];
...
export async function installHostHooks(
  ...
  if (options.launcher && options.host !== "codex") {
    throw new LedgerError("invalid-argument", "--launcher applies to --host codex", { host: options.host });
  }
  ...
  const hookPath = hostHookFiles[options.host].path;
  const existing = await readProjectFile(workspace, hookPath);
  // Codex hooks keep the form the file already uses unless --launcher says otherwise.
  const hadLauncher = options.host === "codex" &&
    ledgerHookPrefixes(parseExistingJson(existing, hookPath)).includes(hookLauncherCommand);
  const launcher = options.host === "codex" && (options.launcher ?? hadLauncher);
  const file = hostHookFile(options.host, command, launcher);
  const content = renderHostHooks(options.host, launcher ? hookLauncherCommand : command, existing);
  ...
  let launcherResult: HookLauncherResult | undefined;
  if (launcher || hadLauncher) {
    const script = await readProjectFile(workspace, hookLauncherPath);
    const expectedHash = script === undefined ? null : hashFileContent(script);
    if (launcher) {
      const current = script !== undefined && isCurrentHookLauncher(script, command);
      if (!current) changes.push({ path: hookLauncherPath, content: renderHookLauncher(command), expectedHash });
      launcherResult = { path: hookLauncherPath, state: current ? "current" : "written" };
    } else if (script !== undefined && hookLauncherTarget(script) !== undefined) {
      // --launcher=false: the hooks run the command again, so the script Ledger wrote goes.
      changes.push({ path: hookLauncherPath, delete: true, expectedHash });
      launcherResult = { path: hookLauncherPath, state: "removed" };
    }
  }
  ...
  const nextSteps = launcher && !changed
    ? file.nextSteps.map((step) => (step === codexTrustStep ? codexKeptTrustStep : step))
    : [...file.nextSteps];
  ...
  const base = {
    ...
    events: file.events,
    changed,
    ...(launcherResult ? { launcher: launcherResult } : {}),
    nextSteps,
  };
  if (options.dryRun) {
