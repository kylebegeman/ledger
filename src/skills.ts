...
export function renderLedgerSkill(workspace: LedgerWorkspace): string {
  ...
  return [
    ...
    "## While working",
    "",
    `- \`${command} session note "<fact>"\` records something the next session should know; \`--section Next\` records a follow-up; \`--section Summary\` states the intent of the work and titles the receipt the hooks draft.`,
    `- Host hooks (installed with \`${command} hooks install\`) already track the paths you touch on the active session record and draft a receipt when you stop. The draft is named in a notice on your next prompt and as \`Linked receipt:\` in the session start context.`,
    "",
    "## When work is done",
    "",
    `- With hooks installed, finish the hook-drafted receipt named in the prompt notice and the session start context; do not create another with \`${command} new\`.`,
    `- \`${command} new "<title>" --from-diff --area <area>\` drafts a change entry from the Git diff in repositories without hooks.`,
    ...
    `- \`${command} ready\` must pass before a draft is marked \`landed\`; it reports the drafted default title, TODO markers, template placeholders, missing verification, and unreviewed docs impact with line numbers.`,
    ...
    "",
    `- Mid-turn: nothing is drafted yet, so \`${command} ci\` reports the files this turn changed as uncovered and names the session whose hook will draft their receipt. Keep working; do not create a receipt for them.`,
    ...
    `- Finished: you retitle it and fill it in, by editing the file or with \`${command} update <id> --title "<title>" --section "Summary=<text>"\`, then \`${command} ready\` gates it.`,
