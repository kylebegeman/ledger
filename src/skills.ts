...
export function renderLedgerSkill(workspace: LedgerWorkspace): string {
  ...
  return [
    ...
    "# Ledger",
    "",
    `Ledger keeps ${project}'s change memory as Markdown records under \`.ledger/\`: change entries with invariants and verification, backlog items, decisions, releases, and short-lived session records. Every command accepts \`--json\` and returns a machine envelope; the same operations exist as MCP tools when \`ledger mcp\` or \`ledger serve --api\` is running.`,
    "",
